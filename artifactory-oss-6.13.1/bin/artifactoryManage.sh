...
to your /etc/security/limits.conf file."
        ...
        # Create the PID file
        local javaPid=$(ps -ef | grep java | grep artifactory | awk '{print $2}')
        if [ -z "${javaPid}" ]; then
            errorArtHome "Unable to get java PID"
        fi
        echo "Artifactory running with PID ${javaPid}"
        # Make sure PID file directory is owned by root (https://access.redhat.com/errata/RHSA-2019:2091)
        chown root:root $(dirname ${ARTIFACTORY_PID})
        echo ${javaPid} > ${ARTIFACTORY_PID}
